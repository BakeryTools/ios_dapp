...
class PrivateBalanceFetcher: PrivateBalanceFetcherType {
    ...
    private func getERC20Balance(for address: TBakeWallet.Address, completion: @escaping (ResultResult<BigInt, AnyError>.t) -> Void) {
    ...
    private func getERC875Balance(for address: TBakeWallet.Address, completion: @escaping (ResultResult<[String], AnyError>.t) -> Void) {
        withRetry(times: numberOfTimesToRetryFetchContractData) { [weak self] triggerRetry in
            guard let strongSelf = self else { return }
    ...
    private func getERC721ForTicketsBalance(for address: TBakeWallet.Address, completion: @escaping (ResultResult<[String], AnyError>.t) -> Void) {
        withRetry(times: numberOfTimesToRetryFetchContractData) { [weak self] triggerRetry in
            guard let strongSelf = self else { return }
    ...
    private func getERC721Balance(for address: TBakeWallet.Address, completion: @escaping (ResultResult<[String], AnyError>.t) -> Void) {
        withRetry(times: numberOfTimesToRetryFetchContractData) { [weak self] triggerRetry in
            guard let strongSelf = self else { return }
    ...
    private func updateNonOpenSeaNonFungiblesBalance(erc721ContractsNotFoundInOpenSea contracts: [TBakeWallet.Address], tokens: [Activity.AssignedToken], group: DispatchGroup) {
    ...
    private func updateNonOpenSeaNonFungiblesBalance(contract: TBakeWallet.Address, tokens: [Activity.AssignedToken], tokensDatastore: PrivateTokensDatastoreType) -> Promise<Void> {
    ...
    private func fetchNonFungibleJson(forTokenId tokenId: String, address: TBakeWallet.Address, tokens: [Activity.AssignedToken]) -> Guarantee<String> {
        firstly {
    ...
    private func fetchTokenJson(forTokenId tokenId: String, uri originalUri: URL, address: TBakeWallet.Address, tokens: [Activity.AssignedToken]) -> Promise<String> {
    ...
    private func updateOpenSeaNonFungiblesBalanceAndAttributes(contractToOpenSeaNonFungibles: [TBakeWallet.Address: [OpenSeaNonFungible]], tokens: [Activity.AssignedToken], group: DispatchGroup) {
