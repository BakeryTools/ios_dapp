...
class GetENSAddressCoordinator {
    ...
    private static var resultsCache = [ENSLookupKey: TBakeWallet.Address]()
    ...
    func getENSAddressFromResolver(
            for input: String,
            completion: @escaping (Result<TBakeWallet.Address, AnyError>) -> Void
    ) {

        //if already an address, send back the address
        if let ethAddress = TBakeWallet.Address(string: input) {
                    ...
                    callSmartContract(withServer: self.server, contract: TBakeWallet.Address(address: resolver), functionName: function.name, abiString: function.abi, parameters: [node] as [AnyObject]).done { result in
                        if let ensAddress = result["0"] as? EthereumAddress {
                            if Constants.nullAddress.sameContract(as: ensAddress) {
                                ...
                                let adress = TBakeWallet.Address(address: ensAddress)
    ...
    func queueGetENSOwner(for input: String, completion: @escaping (Result<TBakeWallet.Address, AnyError>) -> Void) {
        let node = input.lowercased().nameHash
        if let cachedResult = cachedResult(forNode: node) {
            completion(.success(cachedResult))
            return
