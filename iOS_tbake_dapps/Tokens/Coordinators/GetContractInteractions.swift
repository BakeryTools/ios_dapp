...
class GetContractInteractions {
    ...
    func getErc20Interactions(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil) -> Promise<[TransactionInstance]> {
        functional.getErc20Interactions(address: address, server: server, startBlock: startBlock, queue: queue)
    }

    func getErc721Interactions(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil) -> Promise<[TransactionInstance]> {
        functional.getErc721Interactions(address: address, server: server, startBlock: startBlock, queue: queue)
    }

    func getContractList(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil, erc20: Bool) -> Promise<([TBakeWallet.Address], Int?)> {
...
extension GetContractInteractions.functional {
    ...
    static func getErc20Interactions(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil, queue: DispatchQueue) -> Promise<[TransactionInstance]> {
        ...
        //TODO queue?
            ...
            let transactions: [TransactionInstance] = filteredResult.map { result in
                ...
                let localizedTokenObj = LocalizedOperationObjectInstance(
                        ...
                        contract: TBakeWallet.Address(uncheckedAgainstNullAddress: transactionJson["contractAddress"].stringValue),
    ...
    static func getErc721Interactions(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil, queue: DispatchQueue) -> Promise<[TransactionInstance]> {
    ...
    static func getContractList(address: TBakeWallet.Address, server: RPCServer, startBlock: Int? = nil, erc20: Bool) -> Promise<([TBakeWallet.Address], Int?)> {
        ...
        //TODO queue?
            ...
            let nonEmptyContracts = contracts
                    .map { $0.0 }
                    .filter { !$0.isEmpty }
            let uniqueNonEmptyContracts = Set(nonEmptyContracts).compactMap { TBakeWallet.Address(uncheckedAgainstNullAddress: $0) }
