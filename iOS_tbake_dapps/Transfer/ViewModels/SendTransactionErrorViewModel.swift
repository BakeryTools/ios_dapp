...
extension SendTransactionNotRetryableError {
    var faqEntry: (url: URL, title: String)? {
        switch self {
        case .insufficientFunds:
            guard let url = URL(string: "https://exchange.pancakeswap.finance/#/swap") else { return nil }
            return (url: url, title: R.string.localizable.tokenTransactionConfirmationErrorLinkTitleInsufficientFundsError())
        case .nonceTooLow:
