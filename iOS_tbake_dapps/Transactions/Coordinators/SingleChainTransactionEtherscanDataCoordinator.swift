...
class SingleChainTransactionEtherscanDataCoordinator: SingleChainTransactionDataCoordinator {
    ...
    private var contractsToAvoid: [TBakeWallet.Address] {
    ...
    private func filterTransactionsToPullContractsFrom(_ transactions: [TransactionInstance]) -> Promise<(transactions: [TransactionInstance], contractTypes: [TBakeWallet.Address: TokenType])> {
        Promise { seal in
            let contractsToAvoid = self.contractsToAvoid
            let filteredTransactions = transactions.filter {
                if let toAddressToCheck = TBakeWallet.Address(string: $0.to), contractsToAvoid.contains(toAddressToCheck) {
                    return false
                }
    ...
    private func fetchOlderTransactions(for address: TBakeWallet.Address) {
