...
@objc public class OTPFieldView: UIView {
    ...
    public func updateErrorUI() {
        // Forcefully try to make otp field underline red color
        DispatchQueue.main.async {
            (self.viewWithTag(1) as? OTPTextField)?.shapeLayer.strokeColor = UIColor.red.cgColor
            (self.viewWithTag(2) as? OTPTextField)?.shapeLayer.strokeColor = UIColor.red.cgColor
            (self.viewWithTag(3) as? OTPTextField)?.shapeLayer.strokeColor = UIColor.red.cgColor
            (self.viewWithTag(4) as? OTPTextField)?.shapeLayer.strokeColor = UIColor.red.cgColor
        }
    }
